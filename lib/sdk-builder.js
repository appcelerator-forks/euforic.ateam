...
titaniumVersionReader = new wrench.LineReader(path.join('titanium_mobile', 'build', 'titanium_version.py'));
while(titaniumVersionReader.hasNextLine()) {
    titaniumVersion = titaniumVersionReader.getNextLine().match(/^version = '(.*)'$/);
    if (titaniumVersion) {
      titaniumVersion = titaniumVersion[1];
      break;
    }
}
...
buildProcess = child_process.spawn(command, args, {
